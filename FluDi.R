...
##---------------------------------------
## Title: Script to analyze Influenza Parent-of-Origin Effects, based on PsychoDiallel work
...
good.args <- list()
good.args[[1]] <- list("All", "Flu", "Mock")
good.args[[2]] <- list("D0", "pct_d4")
good.args[[3]] <- list("NULL", "D0")
good.args[[4]] <- list("NULL", "Block")

## Default setting when no arguments passed
if(!(length(args) == 4) | 
	!(args[1] %in% good.args[[1]]) |
	!(args[2] %in% good.args[[2]]) |
	!(args[3] %in% good.args[[3]]) |
	!(args[4] %in% good.args[[4]])){
		  stop("Please provide arguments:
		  		Arg 1: All, Flu, or Mock
		  		Arg 2: D0, or pct_d4
		  		Arg 3: NULL, or D0 
		  		Arg 4: NULL, or Block ")
}
