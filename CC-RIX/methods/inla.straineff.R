...
INLAMethod <- setRefClass("INLAMethod",
                          ...
                          methods = list(
                            ...
                            estimate = function(num.draws=200, 
                              ...
                              while(TRUE){
                                ...
                                if (!add.only & use.dip.lincomb) {
                                  ...
                                  for (i in 1:M) {
                                    ...
                                    this.homozygote[i] <- 2
                                    lc = inla.make.lincomb(idx=this.homozygote)
                                    this.lincomb[i] <- lc
                                  ...
                                  for (i in (M+1):T) {
                                    ...
                                    lc = inla.make.lincomb(idx=this.heterozygote.add, dom.idx=this.heterozygote.dom)
                                    this.lincomb[i] <- lc
                                    names(this.lincomb)[i] <- paste("lc", i, sep="")
                                  }
                                }
                                ...
                                hyper.samples <- inla.hyperpar.sample(n=1000, result=result, intern=FALSE)
                                #latent.samples <- inla.posterior.sample(n=latent.sample.num, result=result, hyper.user.scale=TRUE, use.improved.mean=TRUE)
                                latent.samples <- inla.posterior.sample(n=latent.sample.num, result=result, use.improved.mean=TRUE)
                              ...
                              ROP.effects <- NULL
                              if (!is.null(ROP.fits)) {
                                ROP.effects <- ROP.fits
                              }
                              list(results=results, diplotypes=diplotypes, ROP.effects=ROP.effects, strain.names=strain.names)
                            },
                            ...
                            get.ci.from.inla.marginal = function(marginals) {
                              ...
                              for (i in 1:effect.num) {
                                ...
                                quant.wide.left[i] = inla.qmarginal(0.025, marginals[[i]])
                                quant.wide.right[i] = inla.qmarginal(0.975, marginals[[i]])
                                quant.narrow.left[i] = inla.qmarginal(0.25, marginals[[i]])
                                quant.narrow.right[i] = inla.qmarginal(0.75, marginals[[i]])
                              }
...
get.combined.marginal = function(samples, fieldname, rowidx, mliks, is.lincomb=FALSE) {
  ...
  get.weighted.density <- function(x, is.lincomb) {
    ...
    for (i in 1:S) {
      if (is.lincomb) {
        s[i] = inla.dmarginal(x, samples[[i]]$marginals.lincomb[[fieldname]])
      }
      else {
        s[i] = inla.dmarginal(x, samples[[i]]$marginals.random[[fieldname]][[rowidx]])
      }
    }
    return (weighted.mean(s, mliks))
  }
...
load.ROP.estimates <- function(diploffect.inla) {
  method = get.straineff.method("inla")
  if (!is.null(diploffect.inla$ROP)) {
    return(diploffect.inla$ROP)
  }
  if (is.null(diploffect.inla$ROP)) {
    return(NULL)
  }
