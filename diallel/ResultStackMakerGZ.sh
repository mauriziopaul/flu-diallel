...
##---------------------------------------------
find "$DIR" -type f -name 'StackedTreatDi.csv.gz' >> "${DIR}/MIMP_listCompletedGZ.out"
...
##---------------------------------------------
find "$DIR" -type f -name 'StackedTreatDiPost.csv.gz' >> "${DIR}/MIMP_listPostCompletedGZ.out"
...
##-----------------------------------------------------
find "$DIR" -type f -name 'StackedTreatDiPostPred.csv.gz' >> "${DIR}/MIMP_listPostPredCompletedGZ.out"
...
##---------------------------------------------------
# Unzip MIMP files (including posterior / post pred)
##---------------------------------------------------

while read line; do
	gunzip "$line";
	echo "unzipped file ${line}";
done < "${DIR}/MIMP_listCompletedGZ.out"

while read line; do
	gunzip "$line";
	echo "unzipped file ${line}";
done < "${DIR}/MIMP_listPostCompletedGZ.out"

while read line; do
	gunzip "$line";
	echo "unzipped file ${line}";
done < "${DIR}/MIMP_listPostPredCompletedGZ.out"
